...
public class sim_02 {
    ...
    /*自由流速度(m/sec)*/
    private static final double  vf = 15.0 ;
    /*时间步长(sec)*/
    private static final double  dt = 10 ;
    /*阻塞密度(veh/m)*/
    private static final double  kjam = 0.133 ;
    /*阻塞密度(veh/m)*/
    private static double N = 0,N1=0,N2=0,N3=0 ;
            ...
            ctms.forEach((cc)->{
                switch(cc.c_type){
                    case 0:case 3 :case 4 :
                        ctm_w_gth = div(ctm_w_vf,cc.c_lgth);
                        ctm_vf_gth =div(vf,cc.c_lgth);

                        /*当前元胞的驶入流率*/
                        cc.pos_in = min(cc.c_rate,mul(ctm_w_gth,(cc.c_cap - cc.c_n)));
                        /*当前元胞的驶出流率*/
                        cc.pos_out =min(cc.c_rate, mul(ctm_vf_gth,cc.c_n));
                    ...
                    case 1:
                        ...
                        cc.pos_in = 0;
                        cc.pos_out = mul(ctm_vf_gth,cc.c_n);
                    ...
                    switch (length){
                        case 2:
                            ...
                            if(clk.getFlag()==1){
                                ...
                                N = Math.round(mul(f,dt));
                                if(f>0){
                                    updateRightVehicle(r1, r2,N);/*更新vehicle object*/
                                }
                            }
                            break;
                        case 3:
    ...
    private static void updateRightFlow(orginCell cc) {
        ...
        cc.pos_in = min(cc.c_rate,mul(ctm_w_gth,sub(cc.c_cap,posCn)));
    }
    /**
     *
    ...
    private static double div(double v1, double v2) {
        return MathSupplier.div(v1, v2, 2);
    }
    /**
     * 提供精确的减法运算
     * @Title: sub
